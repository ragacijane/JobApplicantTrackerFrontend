...
import { columns, renderCell } from './UserColumns'
...
export default function UserTable() {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (curentUser)
            filteredUsers = filteredUsers.filter(user => user.username !== curentUser.username)
    ...
    return (
        ...
        >
            ...
            <TableBody items={sortedItems} emptyContent={'No users to display.'}>
                {user => (
                    <TableRow key={user.idKorisnika}>
                        {columnKey => <TableCell>{renderCell(user, columnKey, currentUserType)}</TableCell>}
                    </TableRow>
                )}
            </TableBody>
        </Table>
